...
window.onclick = function (e) {
    ...
    if (selectElement('.navs__back').classList.contains('open') && !e.target.closest('.new__hamburg')) {
        selectElement('.navs__back').classList.remove('open');
    }

    if (selectElement('.navs__inner').classList.contains('open') && !e.target.closest('.new__hamburg')) {
        selectElement('.navs__inner').classList.remove('open');
    }
    
    if (selectElement('.hidden__media').classList.contains('open') && !e.target.closest('.open__link')) {
...
selectElement('.open__link').addEventListener('click', function () {
    sleep(2).then(() => {
